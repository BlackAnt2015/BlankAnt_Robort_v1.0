...
static uint8 gFlagSpeedRset = 0; //代表robort车速是否被远程终端修改过
...
{
    uint8 sysSpeed_L = (gFlagSpeedRset==0) ? gRobortSysSpeed : gRobortSpeed_L;
    uint8 sysSpeed_R = (gFlagSpeedRset==0) ? gRobortSysSpeed : gRobortSpeed_R;
...
{
    gFlagSpeedRset = 1;   //标志为1，代表客户端设定了速度，下次方向调整时使用gRobortSpeed_L/gRobortSpeed_R
